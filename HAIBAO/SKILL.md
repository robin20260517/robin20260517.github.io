...
description: Create high-quality static visual design pieces such as posters, game logos, promotional key art, branding visuals, and concept canvases. Especially suitable for iGaming, slot-game, casino, glossy 3D logo, and high-saturation commercial game art directions.
---
...
Before generating a final design, check whether the user has provided the necessary details.

For logo tasks, required details are:
- logo text / letters
- output size
- background type
- color direction
...
If key details are missing, do not generate immediately.
First reply with a short confirmation brief.

Use this default brief for casino / slot / iGaming logo tasks:
- Design type: game logo
- Logo text: ask user to confirm
- Size: 1200x320
...
Use this short confirmation format:

Please confirm this design brief:
- Logo text:
- Size: 1200x320
- Background: transparent PNG
- Style: glossy 3D iGaming / slot logo
- Color: high-saturation casino palette
- Reference: references folder + direct image URL + Yggdrasil style
- Composition: one-line, main letters large, suffix smaller
- Restrictions: no direct copying, keep money symbols simple

If the user confirms, continue generating.
If the user changes any item, update the brief first, then generate.
